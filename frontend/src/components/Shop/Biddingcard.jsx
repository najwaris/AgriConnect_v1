...
const Biddingcard = ({ data }) => {
  ...
  const handleCompleteClick = async () => {
    try {
      const response = await axios.patch(
        `${server}/bidding/Order-complete/${data._id}`
      );
  ...
  const handleCancelClick = async () => {
    try {
      const response = await axios.patch(
        `${server}/bidding/Order-cancel/${data._id}`
      );
      ...
      <div className="flex-grow ml-3">
        ...
        <div className="mt-2">
          <span className="text-lg font-semibold">
            Bidding Amount: RM {data.highestBid}
          </span>
        </div>
        <div className="mt-3">
          <h4 className="text-md font-bold">Bidder Details:</h4>
          <p className="text-sm font-semibold">
            Bidder: {data.highestBidder.name}
          </p>
          <p className="text-sm font-semibold">
            Phone: +(60){data.highestBidder.phoneNumber}
          </p>
          <p className="text-sm font-semibold">
            Email: {data.highestBidder.email}
          </p>
          <br />
          <p className="text-sm font-semibold">
            Status:{" "}
